...
This repository is the Python SDK (`helixid-sdk-py`) — the SDK itself plus the LangChain, CrewAI, and MCP middleware. This is the code that runs inside other people's agents and services, holding wallets and building presentations, so we treat security reports against it seriously.
...
2. **GitHub Security Advisory:** Use the "Report a vulnerability" button under the **Security** tab of the [HelixID repository](https://github.com/helixid/helix-sdk-py/security/advisories). This opens a private advisory visible only to repository maintainers.
...
- **Wallet and key handling** — encrypted wallet storage, passphrase handling, key serialization, private key leakage through logs, tracebacks, or exception messages
- **Presentation building and verification** — VP construction, local verification paths, replay protection
- **Framework middleware** — credential extraction, injection vectors, and policy-enforcement bypass in the LangChain, CrewAI, and MCP adapters
- **Deserialization** — unsafe parsing of credentials, DID documents, or status lists from untrusted input
- **Supply-chain concerns** — tampered PyPI artifacts, typosquat risk on `helixid-sdk-py`, release signing integrity
...
- **Delegation depth:** Set `max_delegation_depth` explicitly on every credential. The default is conservative; do not disable the check.
...
- **Subscribe to advisories:** Watch this repository with "Releases and security advisories" enabled, or subscribe via RSS to the [Security Advisories feed](https://github.com/helixid/helix-sdk-py/security/advisories).
...
- **GitHub Security Advisories:** [github.com/helixid/helix-sdk-py/security/advisories/new](https://github.com/helixid/helix-sdk-py/security/advisories/new)
