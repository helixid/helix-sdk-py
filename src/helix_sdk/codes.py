...
"""
Error code taxonomy, ported 1:1 from helix-sdk-js's src/core/codes.ts.

Kept as a flat set of string constants (not a Python Enum) so values compare
equal to plain strings coming back from JSON API responses without any
`.value` unwrapping -- this mirrors how the JS side treats ErrorCode as a
string union type, not a runtime enum object.
"""
...
# Prepare/finalize (SDK-API-only payload construction)
